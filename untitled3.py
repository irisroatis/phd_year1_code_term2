...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 21 15:24:53 2023
...
def ss_against_mse(how_many_it, parameters, size_test, size_train, type_transf, extra=None, type_regression = None, alpha = None, want_the_betas = False):
    ...
    for iteration in range(how_many_it):
        ...
        for i in range(how_many_extras - 1):
            ...
            if want_the_betas:
                y_predicted_binned, estimated_betas_0['none'] [i+1, iteration], estimated_betas_1['none'] [i+1, iteration] = predict_regressions(X, y, X_test, 'ridge',a=extra[i],want_the_beta =  True)
            else:
                y_predicted_binned = predict_regressions(X, y, X_test, 'ridge',want_the_beta =  False)
...
max_bin_size = 8.5
extra = np.linspace(0.01, 2000, 30)
...
for i in range(1):
    ...
    plt.title('Progress of regression line as ridge penalty increases, parameters: '+str(parameters))
    plt.show()
