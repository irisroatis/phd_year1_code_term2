...
def roc_logistic(X, y, testing_data, belonging_classes, list_of_thresholds, how_many_times_repeat):
    ...
    for t in list_of_thresholds:
        ...
        for repeat in range(how_many_times_repeat):
            ...
            predicted_classes =  predict_logistic(testing_data[repeat], parameters["beta"], parameters["beta_0"], t)
...
y = np.concatenate((0*np.ones((nt1,1)),np.ones((nt1,1))))
...
y_binned = np.concatenate((0*np.ones((nt1,1)),np.ones((nt1,1))))
